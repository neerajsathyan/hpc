...
int EncryptSeq (int n, char* data_in, char* data_out, char *key, int keySize) 
{  
  int i;
  timer sequentialTime = timer("Sequential encryption");
  
  sequentialTime.start();
  for (i=0; i<n; i++) { data_out[i]=data_in[i] + key[i%keySize]; }
  sequentialTime.stop();
...
int DecryptSeq (int n, char* data_in, char* data_out, char *key, int keySize)
{
  int i;
...
int EncryptCuda (int n, char* data_in, char* data_out, char *key, int keySize) {
    ...
    char* deviceKey = NULL;
    checkCudaCall(cudaMalloc((void **) &deviceKey, keySize * sizeof(char)));
    if (deviceKey == NULL) {
        cout << "could not allocate memory!" << endl;
        return -1;
    }
    ...
    checkCudaCall(cudaMemcpy(deviceKey, key, keySize*sizeof(char), cudaMemcpyHostToDevice));
    memoryTime.stop();

    int blocks = (n/threadBlockSize) + ((n % threadBlockSize) != 0);

    // execute kernel
    kernelTime1.start();
    encryptKernel<<<blocks, threadBlockSize>>>(deviceDataIn, deviceDataOut, n, deviceKey, keySize);
...
int DecryptCuda (int n, char* data_in, char* data_out, char *key, int keySize) {
    int threadBlockSize = 512;
    ...
    // execute kernel
    kernelTime1.start();
    decryptKernel<<<blocks, threadBlockSize>>>(deviceDataIn, deviceDataOut, n, deviceKey, keySize);
    cudaDeviceSynchronize();
    kernelTime1.stop();
