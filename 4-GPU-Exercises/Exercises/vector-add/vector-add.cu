...
void vectorAddCuda(int n, float* a, float* b, float* result) {
	...
	//Grid Geo..
	//dim3 DimGrid(256, 80); //20480 thread blocks per grid
	//dim3 DimBlock(4, 4, 2); //32 threads per block

    // allocate the vectors on the GPU
    float* deviceA = NULL;
    checkCudaCall(cudaMalloc((void **) &deviceA, n * sizeof(float)));
    if (deviceA == NULL) {
        cout << "could not allocate memory at deviceA!" << endl;
        return;
    }
    float* deviceB = NULL;
    ...
    if (deviceB == NULL) {
        ...
        cout << "could not allocate memory at deviceB!" << endl;
        return;
    }
    ...
    if (deviceResult == NULL) {
        ...
        cout << "could not allocate memory at deviceResult!" << endl;
        return;
    }

    timer kernelTime1 = timer("kernelTime1");
    timer memoryTime = timer("memoryTime");
    timer totalTime = timer("totalGPUTime");
    ...
    //vectorAddKernel<<<DimGrid, DimBlock>>>(deviceA, deviceB, deviceResult);
    ...
    cout << "GPU Total time (kernel + memory): \t\t" << totalTime << endl;
}
